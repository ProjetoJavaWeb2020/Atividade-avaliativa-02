...
<title>Categoria rapadura</title>
...
<div class = "conteudo">
		...
		<div class="row">
			...
			<c:forEach items="${listaProdutos}" var="produto">
				<c:if test="${produto.getCategoria() == 'rapadura'}">	
					<div class="col-4">
						<img src="${produto.getImagem() }">
						<p><c:out value="${produto.getNome() }"></c:out> - R$<c:out value="${produto.getPreco() }"></c:out></p>
						<a href="Controller?pagina=adicionarCarrinho&acao=rapadura&id=<c:out value="${produto.getId()}"/>">Adicionar ao Carrinho</a>
					</div>
				</c:if>
			</c:forEach>
